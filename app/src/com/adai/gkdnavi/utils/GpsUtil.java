...
/**
 * Created by huangxy on 2017/5/27 9:25.
 */
...
//        }
    ...
    public static boolean outOfChina(double lat, double lon) {
        InitPolygon();
        boolean isInCHINA = IsPtInPoly(new Point(lat, lon), polygon_All);
        Log.e("9527", "isInCHINA = " + isInCHINA);
        return !isInCHINA;
...
//        if (lon >= 120.1 && lon <= 121.3 && lat >= 22.9 && lat <= 25.4) {
//            double[] gps84 = {lat, lon};
//            double[] bd09 = gcj02_To_Bd09(gps84[0], gps84[1]);
//            return bd09;
//        }
    }

    public static double[] gps84_To_bd09(double lat, double lon, boolean outOfChina) {
        if (outOfChina) {
            return new double[]{lat, lon};
        } else {
            double[] gcj02 = gps84_To_Gcj02(lat, lon);
    ...
    static List<List<Point>> polygon_All = new ArrayList<List<Point>>();
    static List<Point> polygon_Mainland = new ArrayList<>();
    static List<Point> polygon_Taiwan = new ArrayList<>();
    static List<Point> polygon_Hainan = new ArrayList<>();
    static List<Point> polygon_Chongming = new ArrayList<>();

    public static void InitPolygon() {
        Point[] taiwanPoints = new Point[]  //Taiwan
                {
        ...
        polygon_Taiwan = Arrays.asList(taiwanPoints);

        Point[] hainanPoints = new Point[]  //Hainan
                {
        ...
        polygon_Hainan = Arrays.asList(hainanPoints);

        Point[] chongmingPoints = new Point[]  //Chongming
                {
        ...
        polygon_Chongming = Arrays.asList(chongmingPoints);


        Point[] mainlandPoints = new Point[]  //Mainland
                {
        ...
        polygon_Mainland = Arrays.asList(mainlandPoints);

        polygon_All.add(polygon_Mainland);
        polygon_All.add(polygon_Taiwan);
        polygon_All.add(polygon_Hainan);
        polygon_All.add(polygon_Chongming);
    }
     ...
     */
    public static boolean IsPtInPoly(Point point, List<List<Point>> polygon_all) {
        for (int j = 0; j < polygon_all.size(); j++) {
            int N = polygon_all.get(j).size();
            ...
            p1 = polygon_all.get(j).get(0);//left vertex
            for (int i = 1; i <= N; ++i) {//check all rays
                if (p.equals(p1)) {
                    return boundOrVertex;//p is an vertex
                }

                p2 = polygon_all.get(j).get(i % N);//right vertex
                ...
                if (p.x > Math.min(p1.x, p2.x) && p.x < Math.max(p1.x, p2.x)) {//ray is crossing over by the algorithm (common part of)
                    ...
                    if (p.x == p2.x && p.y <= p2.y) {//p crossing over p2
                        Point p3 = polygon_all.get(j).get((i + 1) % N); //next vertex
