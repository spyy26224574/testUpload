...
import com.adai.gkdnavi.fragment.square.AttentionListFragment.OnListFragmentInteractionListener;
 ...
 */
public class AttenttionRecyclerViewAdapter extends RecyclerView.Adapter<AttenttionRecyclerViewAdapter.ViewHolder> {

    private final List<LikeUserBean> mValues;
    private final OnListFragmentInteractionListener mListener;
    private Fragment mContext;

    public AttenttionRecyclerViewAdapter(Fragment context, List<LikeUserBean> items, OnListFragmentInteractionListener listener) {
        mValues = items;
        mListener = listener;
        mContext = context;
    }
    ...
    public void onBindViewHolder(final ViewHolder holder, final int position) {
        ...
        ImageLoaderUtil.getInstance().loadRoundImage(mContext.getContext(), holder.mItem.portrait, R.drawable.default_header_img, holder.head_img);
        ...
        holder.add_attention.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (!((BaseFragment) mContext).checkLogin()) {
                    return;
                }
                if ("Y".equals(holder.mItem.isFocusOn)) {
                            ...
                            if (result != null) {
                                switch (result.ret) {
                                    case 0:
                                        ...
                                        break;
                                    default:
                                        Toast.makeText(mContext.getActivity(), result.message, Toast.LENGTH_SHORT).show();
                                        break;
                                }
                            }
                        }
        ...
        holder.head_img.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent personal = new Intent(mContext.getContext(), PersonalPageActivity.class);
                personal.putExtra("userid", holder.mItem.userId);
                personal.putExtra("position", position);
                mContext.startActivityForResult(personal, PersonalPageActivity.REQUEST_CODE_FOUCS);
            }
        });
    }
