...
public class CameraCommand {
    ...
    public static String PROPERTY_GSENSOR = "GSensor";
...
//
//        if (CameraSniffer.isAPEnable() && (CameraUIConfig.uiType == CameraUIConfig.cdvView)) {
...
//        }
    ...
    private static URL buildRequestUrl(String path, String action, String argumentList) {
        ...
        try {
            if (getCameraIp() != null) {
                MstarCamera.CAM_IP = getCameraIp();
            }
            if (MstarCamera.CAM_IP != null)
                url = new URL("http://" + MstarCamera.CAM_IP + path + "?action=" + action + argumentList);
...
//		arguments[3] = buildArgument(PROPERTY_SETADASYTWO) ;
    ...
    public static URL commandSetUrl(int menuId, String param) {
        String[] arguments = new String[1];
        switch (menuId) {
            case MstarRepository.MENU_ID.menuEV:
                arguments[0] = buildArgument(PROPERTY_EV, param);
                break;
            case MstarRepository.MENU_ID.menuIMAGE_RES:
                arguments[0] = buildArgument(PROPERTY_IMAGE, param);
                break;
            case MstarRepository.MENU_ID.menuWHITE_BALANCE:
                arguments[0] = buildArgument(PROPERTY_AWB, param);
                break;
            case MstarRepository.MENU_ID.menuMTD:
                arguments[0] = buildArgument(PROPERTY_MTD, param);
                break;
            case MstarRepository.MENU_ID.menuVIDEO_RES:
                arguments[0] = buildArgument(PROPERTY_VIDEO, param);
                break;
            case MstarRepository.MENU_ID.menuGST:
                arguments[0] = buildArgument(PROPERTY_GSENSOR, param);
                break;
            default:
                break;
        }
    ...
    public static void asynSendRequest(final URL url, final RequestListener requestListener) {
        ...
        stringRequest.setRetryPolicy(new DefaultRetryPolicy(VOLLEYTIMEOUT, DefaultRetryPolicy.DEFAULT_MAX_RETRIES, DefaultRetryPolicy.DEFAULT_BACKOFF_MULT));
    ...
    public static class SendRequest extends AsyncTask<URL, Integer, String> {
        ...
        protected void onPostExecute(String result) {
            ...
            if (pat != null && pat.isEmpty()) {
                ...
                ToastUtil.showShortToast(R.string.set_failure);
            }
