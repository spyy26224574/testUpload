...
import com.adai.gkdnavi.PanoVideoPreviewActivity;
...
import com.filepicker.imagebrowse.PanoImageActivity;
...
/**
 * Created by huangxy on 2017/8/8 11:14.
 */

public class NovatekFileManagerAdapter extends RecyclerView.Adapter {
    ...
    public NovatekFileManagerAdapter(Activity activity, ArrayList<MinuteFile> minuteFiles, int type) {
        mActivity = activity;
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        switch (viewType) {
            ...
            default:
                ...
                mContentViewHolder = new ContentViewHolder(itemView);
    ...
    private class ContentViewHolder extends RecyclerView.ViewHolder implements MinuteFileDownloadManager.DownloadObserver {
        ...
        private TextView mTvTime, mTvNum, mTvSize, mTvType;
        ...
        public ContentViewHolder(View itemView) {
            ...
            mProgressCircleView.setVisibility(mFileType == FileManagerConstant.TYPE_REMOTE_NORMAL_VIDEO ||
                    mFileType == FileManagerConstant.TYPE_REMOTE_URGENCY_VIDEO || mFileType == FileManagerConstant.TYPE_REMOTE_MONITOR_VIDEO ? View.VISIBLE : View.GONE);
...
//                                    boolean camera_pano = SpUtils.getBoolean(VLCApplication.getAppContext(), CameraConstant.CAMERA_PANO, true);
                                    if (mMinuteFile.fileDomains.get(0).fpath.contains("_A")) {
                                        PanoImageActivity.newInstance(mActivity, mMinuteFile.fileDomains.get(0).fpath, PanoImageActivity.MODE_LOCAL);
                                    } else {
...
//                                        mActivity.startActivityForResult(intent, NovatekFileManagerFragment.REQUEST_FILE_DELETE);
                                        Intent intent = new Intent();
                                        ArrayList<String> values = new ArrayList<>();
                                        values.add(mMinuteFile.fileDomains.get(0).fpath);
                                        intent.putExtra(PictureBrowseActivity.KEY_MODE, PictureBrowseActivity.MODE_LOCAL);
                                        intent.putStringArrayListExtra(PictureBrowseActivity.KEY_TOTAL_LIST, values);
...
//                                        }
//                                    }
                                    videoDomains.add(mMinuteFile.fileDomains.get(0));
//                                    boolean camera_pano = SpUtils.getBoolean(VLCApplication.getAppContext(), CameraConstant.CAMERA_PANO, true);
                                    if (mMinuteFile.fileDomains.get(0).fpath.contains("_A")) {
//                                        int position = videoDomains.indexOf(mMinuteFile.fileDomains.get(0));
                                        intent.putExtra(PanoVideoPreviewActivity.KEY_POSTION, 0);
                                        intent.putExtra(PanoVideoPreviewActivity.KEY_FILES, videoDomains);
                                        intent.putExtra(PanoVideoPreviewActivity.KEY_TYPE, 1);
                                        intent.setClass(mActivity, PanoVideoPreviewActivity.class);
                                        mActivity.startActivity(intent);
                                    } else {
//                                        int position = videoDomains.indexOf(mMinuteFile.fileDomains.get(0));
                                        intent.putExtra(VideoPreviewActivity.KEY_POSTION, 0);
                                        intent.putExtra(VideoPreviewActivity.KEY_FILES, videoDomains);
                                        intent.putExtra(VideoPreviewActivity.KEY_TYPE, 1);
                                        intent.setClass(mActivity, VideoPreviewActivity.class); //VideoPreviewActivity
                                        mActivity.startActivity(intent);

                                    }
                                }
                                break;
                            case FileManagerConstant.TYPE_REMOTE_MONITOR_VIDEO:
...
//                                    boolean camera_pano = SpUtils.getBoolean(VLCApplication.getAppContext(), CameraConstant.CAMERA_PANO, true);
                                    ...
                                    if (mMinuteFile.fileDomains.get(0).fpath.contains("_A")) {
                                        PanoImageActivity.newInstance(mActivity, mMinuteFile.fileDomains.get(0).getDownloadPath(), PanoImageActivity.MODE_NETWORK);
                                    } else {
                                        ...
                                        fileList.add(mMinuteFile.fileDomains.get(0).getDownloadPath());
//                                        int titlePosition = mPosition;
//                                        int nextTitlePosition = mPosition;
//                                        for (; !mMinuteFiles.get(titlePosition).isTitle; titlePosition--) {
//
...
//                                        for (int i = titlePosition + 1; i < nextTitlePosition; i++) {
//                                            if (mMinuteFiles.get(i).fileDomains.get(0).isPicture) {
//                                                fileList.add(mMinuteFiles.get(i).fileDomains.get(0).getDownloadPath());
//
//                                            }
//                                        }
//                                        int position = fileList.indexOf(mMinuteFile.fileDomains.get(0).getDownloadPath());
                                        ...
                                        mActivity.startActivityForResult(picture, NovatekFileManagerFragment.REQUEST_FILE_DELETE);
                                    }
...
//                                    boolean camera_pano = SpUtils.getBoolean(VLCApplication.getAppContext(), CameraConstant.CAMERA_PANO, true);
                                    if (mMinuteFile.fileDomains.get(0).fpath.contains("_A")) {
                                        intent.putExtra(PanoVideoPreviewActivity.KEY_POSTION, 0);
                                        intent.putExtra(PanoVideoPreviewActivity.KEY_FILES, videoDomains);
                                        intent.putExtra(PanoVideoPreviewActivity.KEY_TYPE, 0);
                                        intent.setClass(mActivity, PanoVideoPreviewActivity.class);
                                        mActivity.startActivity(intent);
                                    } else {
                                        intent.putExtra(VideoPreviewActivity.KEY_POSTION, 0);
...
//            mIvLock.setVisibility(mMinuteFile.fileDomains.get(0).attr == 33 ? View.VISIBLE : View.GONE);
            if (mFileType == FileManagerConstant.TYPE_REMOTE_NORMAL_VIDEO || mFileType == FileManagerConstant.TYPE_REMOTE_URGENCY_VIDEO || mFileType == FileManagerConstant.TYPE_REMOTE_MONITOR_VIDEO) {
...
//            mTvNum.setVisibility(mMinuteFile.fileDomains.size() == 1 ? View.GONE : View.VISIBLE);
            ...
            if (mMinuteFiles.get(position).fileDomains.get(0).fpath.contains("_A")) {
                mTvType.setText("A");
            } else if (mMinuteFiles.get(position).fileDomains.get(0).fpath.contains("_C")) {
                mTvType.setText("C");
            } else {
                mTvType.setText("B");
            }
...
//            mTvSelect.setText(mMinuteFiles.get(position).isTitleSelected ? mActivity.getString(R.string.cancel_select) : mActivity.getString(R.string.select));
    ...
    public String getVideoPath(FileDomain file, int type) {
